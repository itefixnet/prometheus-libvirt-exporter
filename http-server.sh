#!/bin/bash
#
# Libvirt Prometheus Exporter HTTP Server
...
LISTEN_PORT="${LISTEN_PORT:-9178}"
LISTEN_ADDRESS="${LISTEN_ADDRESS:-0.0.0.0}"
EXPORTER_SCRIPT="${EXPORTER_SCRIPT:-${SCRIPT_DIR}/libvirt-exporter.sh}"
...
start_server() {
    log "Starting Libvirt Prometheus Exporter HTTP Server"
    ...
    local handler_script="/tmp/libvirt_handler_$$"
...
case "\$path" in
    ...
    "/health")
        ...
        ;;
    "/")
        html="<!DOCTYPE html><html><head><title>Libvirt Exporter</title></head><body><h1>Libvirt Prometheus Exporter</h1><p><a href=\"/metrics\">Metrics</a> | <a href=\"/health\">Health</a></p></body></html>"
...
stop_server() {
    log "Stopping Libvirt Prometheus Exporter HTTP Server"
...
case "${1:-start}" in
    ...
    "test")
        ...
        ;;
    "version")
        echo "Libvirt Exporter HTTP Server v1.0.0"
        ;;
    "help"|"-h"|"--help")
        ...
        echo "  LISTEN_PORT       - HTTP server port (default: 9178)"
        echo "  LISTEN_ADDRESS    - HTTP server bind address (default: 0.0.0.0)"
        echo "  EXPORTER_SCRIPT   - Path to exporter script (default: ./libvirt-exporter.sh)"
